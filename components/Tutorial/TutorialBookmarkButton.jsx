...
export default function TutorialBookmark({
  tutorialId,
  onChange,
  showIconOnly = false,
  listView = false,
  ...
  useEffect(() => {
    const checkBookmarkStatus = async () => {
      ...
      try {
        const res = await fetch("/api/tutorials/check-tutorial-bookmark", {
          method: "POST",
  ...
  const toggleBookmark = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/tutorials/toggle-tutorial-bookmark", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tutorialId,
      ...
      if (data.bookmarked === true) {
        setBookmarked(true);
        onChange?.(tutorialId, true);
      } else if (data.bookmarked === false) {
        setBookmarked(false);
        onChange?.(tutorialId, false);
      }
    } catch (error) {
      console.error("Error toggling bookmark:", error);
    }
  }, [isSignedIn, user, tutorialId, onChange]);
